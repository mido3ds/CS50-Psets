...
char* indexes(const char* path)
{
    // correct path, add dot
    char* correct_path = malloc(strlen(path) + 2);
    strcat(correct_path, ".");
    strcat(correct_path, path);
    correct_path[strlen(correct_path)] = '\0';

    // open dir
    DIR* dir = opendir(correct_path);
    if (dir == NULL)
    {
    ...
    // linear search into files
    while ((dp = readdir(dir)) != NULL)
    {
        file_name = dp->d_name;
        if (strcasecmp(file_name, "index.php") == 0 || strcasecmp(file_name, "index.html") == 0)
        {
            // close file
            if (closedir(dir) == -1)
            {
                printf("\nIndex found but\nCouldn't close directory!\n");
            }

            // store new path
            char* new_path = malloc(strlen(correct_path) + strlen(file_name) + 1);
            strcat(new_path, correct_path);
            strcat(new_path, file_name);
            new_path[strlen(new_path)] = '\0';      // put end

            // return it without dot
            return ++new_path;
        }
    } 
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    {
        // enlarge buffer
        BYTE* temp_ptr = realloc(buffer, ++bytes_num);
        if (temp_ptr == NULL)
            return false;
        else
            buffer = temp_ptr;
        ...
        // read until EOF
        if (fread(buffer, sizeof(BYTE), 1, file) == 0)
        {
            bytes_num--;
            break;
        }
    ...
    // return content, change its pointer 
    *content = buffer;
    if (*content == NULL)
    {
        return false;
    ...
    // return length, change its value
    *length = bytes_num; 
...
const char* lookup(const char* path)
{
    ...
    //compare with availaple extensions
...
{
    // parse all
    char method[3], http_version[8];
    if (sscanf(line, "%s %s %s \r\n", method, abs_path, http_version) != 3)
    {
        error(400);
        return false;
    }
    ...
    // ensure request_target starts with /
    else if (abs_path[0] != '/')
    {
        error(501);
        return false;
    ...
    // ensure request_target contains no "
    else if (strchr(abs_path, '\"') != NULL)
    {
    	error(400);
    	return false;
    }
    ...
    // get ? in path
    char* q = strrchr(abs_path, '\?');

     // query is the next char, whether it is \0 or the actual query
     if (q == NULL)
     {
         query[0] = '\0';
     }
